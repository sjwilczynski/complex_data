...
```
```{r}
anova(c(simplest.un.model.lin, simplest.un.model.quad, simplest.un.model.cube))
```
...
```{r}
explained_var <- function(model, data.set) {
...
analyse_models <- function(data.set=kidney, normalize=F, data.split = 0.8){
  ...
  n.growth.model.cube <- lme(MDRD~(time + I(time^2)+I(time^3))*therapy  + donor.age +  bpl.drugs , random =~time + I(time^2)+I(time^3)|factor(id), data = lkidney_train)
  ...
  all_models <- list(n.growth.model, n.growth.model.cube, n.simplest.un.model, n.simplest.quad.model, n.un.full.model)
  
  AICs <- anova(n.growth.model, n.growth.model.cube, n.simplest.un.model, n.simplest.quad.model, n.un.full.model)$AIC
  vars <- sapply(all_models, explained_var, rbind(lkidney_train, lkidney_test))
  errors <- sapply(all_models, analyse_model, lkidney_train, lkidney_test)
  ...
  if(normalize){
    ...
    test.err <- mean((real.values.test-pr.nn.test$net.result)**2)
    pr.nn.train <- compute(nn, lkidney_train[,c(2, 8, 9)])
    train.err <- mean((real.values.train-pr.nn.train$net.result)**2)
    result[,(ncol(result)+1)] <- c(NA, NA, train.err, test.err)
  }
  
  result
}
```
AIC heura wiec test bardziej odpowiedni,
nie dziwimy sie ze mixed duzy procent, relacja?



```{r}
result <- analyse_models()
```
```{r}
resultN <- analyse_models(normalize = T)
```
