...
```{r}
explained_var(model, data.set) {
  var1 <- var(data.set$MDRD)
  var2 <- var(residuals(model))
  (1-var2/var1)*100
}

analyse_model <- function(model, train.data, test.data){
...
analyse_models <- function(data.set=kidney, normalize=F, data.split = 0.8){
  smp_size <- floor(data.split * nrow(data.set))
  ...
  kidney_train <- nkidney[train_ind, ]
  kidney_test <- nkidney[-train_ind, ]
  
  lkidney_train <- reshape(kidney_train, direction = "long", varying = list(9:16), v.names = c("MDRD"), times = ntime)
  
  lkidney_test <- reshape(kidney_test, direction = "long", varying = list(9:16), v.names = c("MDRD"), times = ntime)
  
  # fitting models
  
  n.growth.model <- lme(MDRD~(time + I(time^2))*therapy  + donor.age +  bpl.drugs , random =~time + I(time^2)|factor(id), data = lkidney_train)
  ...
  n.simplest.quad.model <- gls(MDRD~(ltime+I(ltime^2))+therapy+donor.age+bpl.drugs,
  correlation=corSymm(form= ~1 | id),
  weights=varIdent(form= ~1 | time),
  data=lkidney_train)
  
  AICs <- anova(n.growth.model, n.simplest.un.model, n.un.full.model, n.simplest.quad.model)
  vars <- 
}
```
...
```
```{r}
scaled_time <- scale(c(1/4,1,3,6,12,24,36,60))
```
...
lkidney_train <- reshape(kidney_train, direction = "long", varying = list(9:16), v.names = c("MDRD"), times = scaled_time)

lkidney_test <- reshape(kidney_test, direction = "long", varying = list(9:16), v.names = c("MDRD"), times = scaled_time)


growth.model.predict <- lme(MDRD~(time + I(time^2))*therapy  + donor.age +  bpl.drugs , random =~time + I(time^2)|factor(id), data = lkidney_train)
...
un.model.predict <- gls(MDRD~donor.age+bpl.drugs+recipient.age+discrepancy.AB+discrepancy.DR+CIT+factor(time)*(diabetes+therapy),
correlation=corSymm(form= ~1 | id),
weights=varIdent(form= ~1 | time),
data=lkidney_train)

simplest.quad.model.predict <- gls(MDRD~(ltime+I(ltime^2))+therapy+donor.age+bpl.drugs,
correlation=corSymm(form= ~1 | id),
weights=varIdent(form= ~1 | time),
data=lkidney_train)
```
...
print(paste("Test variance=",var.test))
print(paste("Fixed effects test error=",mean((real.values.test-predictions.gls.test)**2)))
print(paste("Mixed effects test error=",mean((real.values.test-predictions.lme.test)**2)))

print(paste("Train variance=",var.train))
print(paste("Fixed effects residuals=",var.gls.residuals))
print(paste("Fixed effects train error=",mean((real.values.train-predictions.gls.train)**2)))
print(paste("Mixed effects residuals=",var.lme.residuals))
print(paste("Mixed effects train error=",mean((real.values.train-predictions.lme.train)**2)))
```


```{r}
f <- as.formula("MDRD~donor.age+bpl.drugs+time")
nn <- neuralnet(f, data=lkidney_train, hidden=c(9,3),algorithm = "rprop+",
                err.fct = "sse",threshold = 0.1, act.fct = "logistic", linear.output=T)
```

```{r}
pr.nn.test <- compute(nn, lkidney_test[,c(2, 8, 9)])
print(paste("NN test error=",mean((real.values.test-pr.nn.test$net.result)**2)))
pr.nn.train <- compute(nn, lkidney_train[,c(2, 8, 9)])
print(paste("NN train error=",mean((real.values.train-pr.nn.train$net.result)**2)))
```
