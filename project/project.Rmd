...
xy.kidney <- reshape(kidney[1:10,], direction = "long", varying = list(9:16), v.names = c("MDRD"), times = c(1/4,1,3,6,12,24,36,60))
xy.kidney$ltime <- log(xy.kidney$time)

xyplot(MDRD ~ time | id, xy.kidney, type=c("g","p","r"), xlab = "time since transplant", ylab = "MDRD", pch=19)
xyplot(MDRD ~ ltime | id, xy.kidney, type=c("g","p","r"), xlab = "log(time since transplant)", ylab = "MDRD", pch=19)
...
```{r}
result <- analyse.models()
```
```{r}
resultN <- analyse.models(normalize = T)
```
