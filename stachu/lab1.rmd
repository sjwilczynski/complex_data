...
1. In the first two tasks we compare two different estimators of error variance covariance matrices. We consider unstructured matrices. The first one is REML estimator:
```{r}
...
In task 3 and 4 we should choose some assumptions about the structure of the variance covariance matrix from the first task. Unfortunately, at the first glance the matrix doesn't have any structure at all. The only clearly visible thing is that the variances are getting bigger with time. First we try to test if assumptions of equal variance is plausible. From the unstructured variance covariance matrix it does not look so. Therefore we choose parameters \textit{correlation=corSymm(form= ~1 | id),
weights=varIdent()}. We received following output (to compare the models we use \textit{anova} function, which performs likelihood ratio test):
...
Next we choose some better assumption: variances are powers of \textit{time} variable. we use parameters: \textit{correlation=corSymm(form= ~1 | id),
...
print(anova(lead.cat.new, lead.cat))
```
Here the p-value is just above significance level - we do not reject the null hypothesis and such assumption about the such structure of variance covariance matrix is believable. We further extend this approach just by relaxing the structure. We again use \textit{varPower} but this time the multiplication constants in the powers can be different for different \textit{time.cat} factor levels. We use parameters: \textit{correlation=corSymm(form= ~1 | id),
weights=varPower(form = ~time+1 | factor(time.cat))}:
```{r}
lead.cat.new <- gls(y~factor(time.cat),
correlation=corSymm(form= ~1 | id),
weights=varPower(form = ~time+1 | factor(time.cat)),
...
print(anova(lead.cat.new, lead.cat))
```
Now p-value is very high. We can clearly say that such assumptions about the covariance matrix does not weaken our model.

Due to the problem with seeing a structure in correlations, we checked many different possible correlations structure. However none of them seemed good enough. For example for parameters \textit{correlation=corCompSymm(form= ~1 | id),
weights=varIdent(form = ~1 | factor(time.cat))} we got:
```{r}
lead.cat.new <- gls(y~factor(time.cat),
...
## Task 5
In this task we compare confidence intervals for means at different time points in two settings: taking into account correlations and the opposite. In this case these confidence intervals are the same as confidence intervals for coefficients $\beta$, because our explanatory variables are just levels of a factor \textit{time.cat}. To get them from \textit{gls} and the standard ones, we use \textit{intervals} and \textit{confint} respectively. We expect to get narrower CIs for model which better explains our data, so the model which takes into account the correlation.  
```{r}
