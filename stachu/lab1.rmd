...
Next we choose some better assumption: variances are powers of \textit{time} variable. we use parameters: \textit{correlation=corSymm(form= $\sim$1 | id),
weights=varPower(form = $\sim$time+1)}:
```{r}
lead.cat.new <- gls(y~factor(time.cat),
correlation=corSymm(form= ~1 | id),
